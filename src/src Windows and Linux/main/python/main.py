...
class MainWindow(QtWidgets.QApplication, Ui_Weather, ApplicationContext):
	...
	def update_cur_data(self,cur):
		self.cur_img.setPixmap(QtGui.QPixmap(self.get_resource(F"imgs/{cur['icon']}.png")))
	...
	def first_update_forecast(self, forecast):
		...
		for x in range(0,len(forecast)):
			...
			img.setGeometry(QtCore.QRect(10, 37, 50, 50))
			...
			img.setText("")
			img.setPixmap(QtGui.QPixmap(self.get_resource(f"imgs/{forecast[x]['icon']}.png")))
			img.setIndent(0)
			#img.setObjectName("img")
			desc = QtWidgets.QLabel(widget)
			desc.setGeometry(QtCore.QRect(0, 78, 70, 119))
			font = QtGui.QFont()
			font.setFamily("Lucida Grande")
